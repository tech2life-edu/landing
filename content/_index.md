---
banner:
  title: El futuro está en la tecnología. Prepárate para él con Tech2Life.
  content: Toma el control de tu futuro en la industria tech. Nuestro programa de mentorías y trabajo simulado está diseñado para enseñarte las habilidades y conocimientos necesarios para ocupar cargos en el apasionante mundo de la tecnología.
  image: /images/banner-art.svg
  button:
    label: "¡Únete a la comunidad de Tech2Life hoy mismo!"
    link: "https://api.whatsapp.com/send?phone=%2B573005792037&text=%C2%A1Hola!%20Estoy%20interesado(a)%20en%20inscribirme%20a%20Tech2Life."
    enable: true

# feature
feature: 
  title: ¿Qué dicen nuestros estudiantes sobre Tech2Life?
  primaryFeature: 
    name: "Alejandro Bohorquez"
    icon: "/images/estudiantes/alejo.png"
    content: "
Entrar en Tech2Life ha sido una grata experiencia en la que he podido mejorar bastante en muchos aspectos distintos, desde aprender un lenguaje de programación, como desenvolverme y darme a entender en un grupo. Claramente, mi vida ha sido mejorada después de esto. La guía que se brinda es muy buena, pues permite obtener resultados mucho más rápido y evitar aprender cosas que no serían tan útiles o destacables en tu CV. Además, me gusta bastante que se obtiene un conocimiento general de cómo es la industria y cómo se trabaja en ella. Es encantador la conexión que se genera al poder compartir espacio con otros aprendices de otras áreas y comprender de qué va su rol y cómo complementa el mío.

Además de esto, es demasiado bueno tener proyectos reales de práctica, pues se abordan de una forma en la cual genera un gran reto en el cual poner a prueba tus conocimientos obtenidos. Lo mejor es que este proyecto es un ambiente seguro, en el cual cometer un error no es motivo de reprender. Es un motivo para seguir aprendiendo y mejorar más.

En general, Tech2Life ha sido una gran experiencia, y me alegra ser parte de esto."
  features:
  - name: "Cristian peña"
    icon: "/images/estudiantes/cristianpena.png"
    linkedin: "https://www.linkedin.com/in/cristiancamilopena/"
    content: "A mis 34 años decidí dar un giro en mi vida profesional, luego de trabajar por más de 10 años en un campo totalmente diferente, comencé a perseguir mi sueño de ingresar a la industria IT. En esta búsqueda encontré en Tech2life el mejor aliado para acelerar mi aprendizaje, con un roadmap de recursos valiosos, mentorías con referentes de la industria y lo mejor, una simulación del entorno laboral para aplicar en proyectos lo aprendido. Cada día reafirmo que tomé la decisión correcta de ingresar a Tech2life"
  - name: "Cristian Rodriguez"
    icon: "/images/estudiantes/cristianespana.png"
    linkedin: "https://www.linkedin.com/in/cristian-camilo-rodriguez-dev/"
    content: "Tech2life me brindó la oportunidad de obtener mi primera experiencia laboral en un entorno simulado, donde pude desarrollar mis habilidades a través de proyectos y rutas de aprendizaje. Además, aprendí valiosas habilidades blandas para liderar y trabajar en equipo en el mundo de las TI. Gracias a esta formación, logré conseguir mi primer empleo con éxito."
  - name: "Karina aviardi"
    icon: "/images/estudiantes/karina.png"
    linkedin: "https://www.linkedin.com/in/karina-avila-/"
    content: "Soy consciente de que sin Tech2Life no estaría en el momento en el que estoy. También aprendí mucho del equipo, al ver sus dificultades o avances, me dio un acercamiento a la realidad. El uso de las herramientas de comunicación internas es algo que me ha ayudado mucho, o sea, ya soy una PjM. 😎🔥 Las mentorías son lo que más resalto. Hablar y perder el miedo en estas mentorías me hace la vida más fácil en mi trabajo. Estudié ingeniería civil y trabajé en esta área durante 4 años, guiando a mi equipo de trabajo, resolviendo inconvenientes y buscando soluciones técnicas para lograr culminar un proyecto en el tiempo y con el presupuesto establecido. Sin embargo, el gremio de la construcción es muy demandante y ahora, con una sonrisa gigante en mi rostro, puedo decir que hago lo mismo sin botas y sin casco, con empatía y conciencia, y con la garantía de seguir aprendiendo herramientas para ser más ágiles y empáticos con las personas que trabajarán conmigo."
  - name: "Alejandro Bohorquez"
    icon: "/images/estudiantes/alejo.png"
    linkedin: "https://www.linkedin.com/in/alejandro-bohorquez-/"
    content: "
Entrar en Tech2Life ha sido una grata experiencia en la que he podido mejorar bastante en muchos aspectos distintos, desde aprender un lenguaje de programación, como desenvolverme y darme a entender en un grupo. Claramente, mi vida ha sido mejorada después de esto. La guía que se brinda es muy buena, pues permite obtener resultados mucho más rápido y evitar aprender cosas que no serían tan útiles o destacables en tu CV. Además, me gusta bastante que se obtiene un conocimiento general de cómo es la industria y cómo se trabaja en ella. Es encantador la conexión que se genera al poder compartir espacio con otros aprendices de otras áreas y comprender de qué va su rol y cómo complementa el mío.

Además de esto, es demasiado bueno tener proyectos reales de práctica, pues se abordan de una forma en la cual genera un gran reto en el cual poner a prueba tus conocimientos obtenidos. Lo mejor es que este proyecto es un ambiente seguro, en el cual cometer un error no es motivo de reprender. Es un motivo para seguir aprendiendo y mejorar más.

En general, Tech2Life ha sido una gran experiencia, y me alegra ser parte de esto."
# services
services:
  - title: "Ruta de estudios especializada en tu área, creada por profesionales de la industria."
    content: "Descubre las habilidades que necesitas para triunfar en el mundo tech. Nuestra ruta de estudios fue diseñada por expertos en tu área para ayudarte a destacar en el mercado laboral."
    images:
      - "/images/rutas/nuestras-rutas.png"
      - "/images/rutas/software-ruta.png"
      - "/images/rutas/ruta-habilidades-blandas.png"
    button:
      enable: false
      label: Check it out
      link: /contact

  - title: "Empodérate en la industria tech: Autonomía respaldada por un equipo comprometido"
    content: "Valoramos tu autonomía y confiamos en que eres capaz de tomar decisiones informadas sobre tu progreso. Te proporcionaremos las herramientas y los recursos necesarios para que puedas explorar y adquirir nuevos conocimientos de acuerdo con tus intereses y metas profesionales. Además, nuestro equipo realizará un seguimiento cercano de tus avances para asegurarnos de que estés alcanzando tus metas de manera efectiva. Estaremos contigo en cada etapa del proceso, brindándote orientación y apoyo personalizados, y ajustando estratégicamente el programa según tus necesidades. Nuestro objetivo es maximizar tu éxito y ayudarte a convertirte en un profesional destacado en la industria tech"
    images: 
      - "/images/scrum.jpg"
    button:
      enable: false
      label: Check it out
      link: /contact
  - title: "Únete a nuestra comunidad con mentores y acelera tu camino hacia el éxito en la industria tech"
    content: "¿Listo para dar el siguiente paso? Únete a nuestra comunidad de mentores expertos y participa en proyectos prácticos que te brindarán una experiencia real en el campo. Aprenderás las últimas tendencias y mejores prácticas, mientras te sumerges en entornos simulados que te prepararán para los desafíos del mundo laboral."
    images:
      - "/images/service-slide-1.png"
      - "/images/service-slide-2.png"
      - "/images/service-slide-3.png"
    button:
      enable: false
      label: Check it out
      link: /contact

  - title: "Aprende a presentarte en entrevistas y mejorar tu perfil profesional."
    content: "Con Tech2Life, adquirirás habilidades para resolver pruebas técnicas y optimizar tu CV y perfil en LinkedIn. Prepárate para presentarte a entrevistas con confianza y destacar entre los candidatos."
    images:
      - "/images/entrevistas/entrevista1.jpg"
      - "/images/entrevistas/entrevista2.jpg"
    button:
      enable: false
      label: Check it out
      link: /contact

# workflow
workflow: 
  title: "Descubre el Mundo del Desarrollo y preparate en:"
  image: "/images/banner.svg"
  programs: 
    - desription: "UX/UI: Diseño Centrado en el Usuario y Creación de Interfaz de Usuario Atractiva."
    - desription: "Project Manager: Planificación, Ejecución y Seguimiento de Proyectos Digitales."
    - desription: "Product Manager: Desarrollo de Productos Exitosos y Estrategias de Mercadotecnia Digital."
    - desription: "Calidad de Software QA: Garantía de Calidad en el Desarrollo de Software y Pruebas Efectivas."
    - desription: "Fullstack Developer: Construcción de Aplicaciones Web Completas desde el Frontend hasta el Backend."
  description: "UX/Ui, Project Manager, Product Manager, Calidad de Software QA, Fullstack Developer"
  

# call_to_action
call_to_action:
  title: Listo para iniciar!
  content: Entra a nuestra comunidad en discord y conoce nuestros servicios
  image: '/images/cta.svg'
  button:
    enable: true
    label: "Ingresa ahora mismo!"
    link: "https://api.whatsapp.com/send?phone=%2B573005792037&text=%C2%A1Hola!%20Estoy%20interesado(a)%20en%20inscribirme%20a%20Tech2Life."
pricing: 
  title: Pricing
  layout: pricing
  draft: false
  plans:
  - title: Plan gratuito
    subtitle: Ingresa a nuestra comunidad!
    price: 0
    type: month
    features:
      - Conocer personas que estan estudiando
      - Podras realizar preguntas a la comunidad
      - Estaras enterado de todos nuestros eventos
    button:
      label: Inicia ya!
      link: "https://api.whatsapp.com/send?phone=%2B573005792037&text=%C2%A1Hola!%20Estoy%20interesado(a)%20en%20inscribirme%20a%20Tech2Life."

  - title: Plan Mensual
    subtitle: Descubre el camino hacia tu éxito profesional.
    price: 15 usd
    type: Mensual
    recommended: true
    features:
      - Soporte de tutoría personalizada.
      - Ruta de estudio 100% enfocada en tus objetivos.
      - Acceso a proyectos prácticos y experiencias de trabajo reales.
      - Participación en eventos exclusivos y oportunidades de networking.
      - Actualizaciones de contenido y acceso a nuevas tendencias y tecnologías.
    button:
      label: Inicia ya!
      link: "https://api.whatsapp.com/send?phone=%2B573005792037&text=%C2%A1Hola!%20Estoy%20interesado(a)%20en%20inscribirme%20a%20Tech2Life."

  - title: Plan Anual
    subtitle: Domina el mundo digital y ahorra 30 usd.
    price: 150 usd
    type: Anual
    recommended: true
    features:
      - Soporte de tutoría personalizada.
      - Ruta de estudio 100% enfocada en tus objetivos.
      - Acceso a proyectos prácticos y experiencias de trabajo reales.
      - Participación en eventos exclusivos y oportunidades de networking.
      - Actualizaciones de contenido y acceso a nuevas tendencias y tecnologías.
    button:
      label: Inicia ya!
      link: "https://api.whatsapp.com/send?phone=%2B573005792037&text=%C2%A1Hola!%20Estoy%20interesado(a)%20en%20inscribirme%20a%20Tech2Life."

  call_to_action:
    title: Quieres saber mas?
    content: Preguntanos cualquier duda que tengas y tambien puedes preguntar por nuestro programa de becas.
    image: '/images/cta.svg'
    button:
      enable: true
      label: "Contactanos"
      link: "https://api.whatsapp.com/send?phone=%2B573005792037&text=%C2%A1Hola!%20Estoy%20interesado(a)%20en%20inscribirme%20a%20Tech2Life"
---